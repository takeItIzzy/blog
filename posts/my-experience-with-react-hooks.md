...
react 文档中有一句话说得很好，**你应该将依赖数组当成语义的保证，而不是性能优化的条件**。这句话在 react 的文档上于 useMemo() 的介绍处被提到，但在 useEffect() 中同样适用，事实上，人们更容易在 useEffect() 中犯这样的错误。
...
监听器思想在于监听特定字段，这不是一个**普适的逻辑**，是只为特定字段服务的，那我认为这其实是某个状态变更逻辑的一部分，而不是一种组件渲染的副作用。把 useEffect() 当监听器，带来的结果就是极大概率依赖写不全，因为你只想在某个（某些）字段变化时做相应，而这个响应却不一定只依赖这个（这些）字段。
...
更多情况下，useEffect() 是被滥用了的。其实 useEffect() 本身就是个不那么常用的 api。事实上，即使是网络请求这个 useEffect() 最常用的场景，在有了 `react-query` 等非常好用的网络请求库之后，也不再需要使用 useEffect() 了，useEffect 已经变成了一个被封装在底层的 hook，而不需要在编写业务代码时使用。
...
依据我的经验，useEffect() 大部分时候被用来控制数据流（似乎是句废话）。小圆中 99% 的场景是用来发起网络请求，即 99% 合理使用 useEffect() 的场景是网络请求，这之中有时也会订阅外部数据源，但此时数据往往是单向流动的，也很少对外部数据源做变更（格式化不属于变更，这应该是在与接口对接的缓冲层就做好的事，不会进入数据流中）；大圆中除小圆部分外的常见场景就不同了，不合理地使用 useEffect() 控制系统内部的数据流，这是经验不足的 react 使用者常做的事，此时数据往往是随意流动。不合理地使用 useEffect() 控制数据流是很危险的事情，是导致数据流混乱、架构设计糟糕、带来更多次组件重绘和写出死循环的罪魁祸首之一（更新：现在 react18 已经发布了，在严格模式下，每个 useEffect() 会被 react 执行两遍，在 useEffect() 内更新状态会变得更加危险和不可控）。
...
上面的代码中，对于 `foo()` 方法使用 useCallback() 是否合理？`MyComponent` 使用 memo 包裹，并且没有 props，也没有 `useContext`，这意味着这个组件的重绘只会由 `stateA` 引起。然后给 foo() 包裹了 useCallback()，将 stateA 作为依赖，这个 useCallback() 就是多余的，每次组件重绘 foo() 都会被重新创建，使用了 useCallback() 并没有让组件的性能更好。
...
另外，从这里我们还能得到一个知识点——不要直接使用引用类型作为 hooks 的依赖。这样会让依赖数组失去作用。如果你需要使用一个对象，那就在依赖中精确到具体的值为基本类型的 key；如果确实做不到精确到 key 的话，你可以使用 useRef() 暂存该对象（其实这样的代码也很恶心）。
...
react 其实性能还不错，可能 jsx 比不上使用模板语法的一些框架，但语言层面的计算依然是很快的。在实际开发过程中，我更倾向于从来不写 memoization。性能问题就等到性能真的出了问题再解决。即使你不写 useCallback() 和 useMemo() 导致组件出现了不必要的重渲染，那通常也问题不大，并且，如果你的数据流很健康，即使不刻意追求使用缓存减少重绘，也通常不会发生太多次的重绘。
...
有一点值得注意，**重渲染一般不是造成性能瓶颈的原因，慢渲染才是**。
...
如果你总在组件中使用 useCallback() 和 useMemo() 意味着什么？这意味着**你的业务逻辑和 UI 耦合了**，你在该写 UI 的地方写了太多的业务逻辑。react 组件只应该专注于视图，组件的唯一目的就是把 UI 渲染出来，其中处理业务逻辑的代码应该越少越好。想想你的组件中，用于管理 UI 的事件有多少，用于承载业务逻辑的事件又有多少？管理 UI 的事件一般是没必要包裹 useCallback() 的，因为缓存的目的是避免组件重绘，而这种事件的目的就是要改变 UI，就是要让组件重绘。承载业务逻辑的事件更需要包裹 useCallback()，因为你既想执行逻辑，又不希望 UI 变化。使用状态管理库或者 useReducer() 管理业务逻辑和数据是更好的方式，你见过在这两处地方加 useCallback() 和 useMemo() 的吗？更多时候，useCallback() 和 useMemo() 也不过是一种亡羊补牢的手段罢了，你的数据和方法声明错了位置，数据流一片混乱，再想通过 memoization 来弥补因此引起的组件重绘。
...
上面刚刚提到过，声明一个组件，唯一的目的就是把 UI 正确地渲染出来。至于业务逻辑，应该越少存在于组件中越好。
...
react 并没有提供很强大的状态管理方案，使用另一个库来管理数据当然是更合理的方式。现在社区有很多优秀的库，目的是为了解决不同的痛点，可以酌情选择。不过大部分时候，我们的业务没有复杂到需要引入一个库，useContext() 和 useReducer() 就可以胜任一部分这个工作。
...
以上三种方案中，拆分 context 最为推荐。当然，还是那个思想——你不必总是在意组件的重绘，如果通常手段使用 useContext() 没有确实引起性能问题，你根本不必要对 useContext() 做如此改造，尽管大胆去用。
